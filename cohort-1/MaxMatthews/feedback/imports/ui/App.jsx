...
class App extends Component {
	...
	editFeedback = feedbackID => {
		const updatedFeedback = window.prompt(
			"What would you like your feedback to be update to?"
		);
	...
	filterStudents = event => {
		...
		const filteredResults = this.props.feedback.filter(feedback => {
			if (feedback.studentID.includes(filterName)) {
				return true;
			} else {
				return false;
			}
				...
				{this.props.currentUser ? (
					<div>
						<form onSubmit={this.createFeedback}>
							<div className="row">
								{/* <div className="col">
									<img src="/cat.jpeg" style={{ width: "100%" }} />
								</div> */}
								...
								<table className="table">
									...
									<tbody>
										{this.state.filteredResults.length > 0
											? this.state.filteredResults.map(feedback => {
